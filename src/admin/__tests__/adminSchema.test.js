...
describe('tests the adminSchema', () => {
  it('snapshot the typedefs', () => {
    expect(schema.typeDefs).toMatchSnapshot()
  })

  it('snapshot the resolver function', () => {
    expect(schema.resolvers).toMatchSnapshot()
  })

  it('tests resolver configuration', () => {
    expect(schema.resolvers.Query.configuration()).toMatchSnapshot
  })
  it ('tests resolver kubernetes=>blacklist ', () => {
    expect(schema.resolvers.Query.kubernetes().blacklist()).toMatchSnapshot()
  })

  it ('tests resolver kubernetes=>clearBlackList ', () => {
    expect(schema.resolvers.Query.kubernetes().clearBlackList()).toBeTruthy()
